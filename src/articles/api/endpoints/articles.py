...
async def create_article(*, db: DbSession, article_in: ArticleCreate) -> Any:
...
async def update_article(*, db: DbSession, article_id: int, article: ArticleUpdate) -> Any:
    article_service = ArticleService(db)
    return await article_service.update(obj_id=article_id, obj=article)


@article_router.delete("/{article_id}", response_model=Article)
async def delete_article(*, db: DbSession, article_id: int) -> Any:
    article_service = ArticleService(db)
